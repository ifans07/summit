<footer class="bg-white">
    <div class="container p-6 mx-auto">
        <div class="lg:flex">
            <div class="w-full -mx-6 lg:w-2/5">
                <div class="px-6">
                    <a href="#">
                        {{-- <img class="w-auto h-12" src="/assets/Logo-SUmmit-FIX.png" alt=""> --}}
                        <h2 class="text-2xl font-bold">Summit Institute for Development</h2>
                    </a>

                    <p class="max-w-sm mt-2 mb-4 text-gray-600">Jl. Sultan Hasanuddin no. 137B <br>Nusa Tenggara Barat - INDONESIA</p>
                    <p class="max-w-sm text-gray-600">Email: human-resource@sid-indonesia.org

                        <br>Phone: +62(370)-644347</p>

                    <div class="flex mt-6 -mx-2">
                        <a href="#" class="mx-2 text-gray-600 transition-colors duration-300 hover:text-blue-500 text-2xl" aria-label="Reddit">
                            <i class="fa-brands fa-instagram"></i>
                        </a>
                    
                        <a href="#" class="mx-2 text-gray-600 transition-colors duration-300 hover:text-blue-500 text-2xl" aria-label="Facebook">
                            <i class="fa-brands fa-square-facebook"></i>
                        </a>
                    
                        <a href="#" class="mx-2 text-gray-600 transition-colors duration-300 hover:text-blue-500 text-2xl" aria-label="Github">
                            <i class="fa-brands fa-linkedin"></i>
                        </a>

                        <a href="#" class="mx-2 text-gray-600 transition-colors duration-300 hover:text-blue-500 text-2xl" aria-label="Github">
                            <i class="fa-brands fa-square-youtube"></i>
                        </a>
                    </div>
                </div>
            </div>

            <div class="mt-6 lg:mt-0 lg:flex-1">
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
                    <div class="group transition-all duration-1000">
                        <div class="flex items-center cursor-pointer">
                            <h3 class="text-gray-700 uppercase">About</h3>
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="h-4 w-4 ml-1">
                                <path stroke-linecap="round" stroke-linejoin="round" d="m19.5 8.25-7.5 7.5-7.5-7.5" />
                            </svg>
                        </div>
                        <div class="hidden group-hover:block md:block">
                            <a href="/about/v0" class="block mt-2 text-sm text-gray-600 hover:underline">Our Story</a>
                            <a href="/about/speciality" class="block mt-2 text-sm text-gray-600 hover:underline">Our Speciality</a>
                            <a href="/about/leadership" class="block mt-2 text-sm text-gray-600 hover:underline">Leadership</a>
                            <a href="/about/our-team" class="block mt-2 text-sm text-gray-600 hover:underline">Our Team</a>
                        </div>
                    </div>

                    <div class="group transition-all duration-1000">
                        <div class="flex items-center cursor-pointer">
                            <h3 class="text-gray-700 uppercase">Our Work</h3>
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="h-4 w-4 ml-1">
                                <path stroke-linecap="round" stroke-linejoin="round" d="m19.5 8.25-7.5 7.5-7.5-7.5" />
                            </svg>
                        </div>
                        <div class="hidden group-hover:block md:block">
                            <a href="/programs-publications" class="block mt-2 text-sm text-gray-600 hover:underline">Programs</a>
                            <a href="/publications" class="block mt-2 text-sm text-gray-600 hover:underline">Publications</a>
                            <a href="/news" class="block mt-2 text-sm text-gray-600 hover:underline">News</a>
                            <a href="/news" class="block mt-2 text-sm text-gray-600 hover:underline">Events</a>
                        </div>
                    </div>

                    <div class="group transition-all duration-1000">
                        <div class="flex items-center cursor-pointer">
                            <h3 class="text-gray-700 uppercase">Programs</h3>
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="h-4 w-4 ml-1">
                                <path stroke-linecap="round" stroke-linejoin="round" d="m19.5 8.25-7.5 7.5-7.5-7.5" />
                            </svg>
                        </div>
                        <div class="hidden group-hover:block md:block">
                            <a href="/programs/dojo" class="block mt-2 text-sm text-gray-600 hover:underline">Genomics Science DOJO</a>
                            <a href="/programs/dojo" class="block mt-2 text-sm text-gray-600 hover:underline">Covneo & PregVax</a>
                            <a href="/programs/dojo" class="block mt-2 text-sm text-gray-600 hover:underline">SUMMIT 10 Year (S10Y) Study</a>
                        </div>
                    </div>

                    {{-- <div class="group transition-all duration-1000">
                        <div class="flex items-center cursor-pointer">
                            <h3 class="text-gray-700 uppercase"><a href="/contact">Contact</a></h3>
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="h-4 w-4 ml-1">
                                <path stroke-linecap="round" stroke-linejoin="round" d="m19.5 8.25-7.5 7.5-7.5-7.5" />
                            </svg>
                        </div>
                        <div class="hidden group-hover:block md:block transition duration-300">
                            <span class="block mt-2 text-sm text-gray-600 hover:underline">+1 526 654 8965</span>
                            <span class="block mt-2 text-sm text-gray-600 hover:underline">example@email.com</span>
                        </div>
                    </div> --}}
                </div>
            </div>
        </div>

        <hr class="h-px my-6 bg-gray-200 border-none">

        <div>
            <p class="text-center text-gray-500">©2025 Summit Institute for Development - All rights reserved.</p>
        </div>
    </div>
</footer>